...
BATCH_SIZE = 500 # number of games to do between saves

def train(default_weighting, weighting, batch_size):
    while True: # train forever
        aip = AIStorage.from_db("database.db")
        ...
        for n,i in enumerate(range(batch_size)): # for each game
            ...
            if b.player1 == b.play(): # if a player has won
                for key in b.player1.moves:
                    aip.add(key, b.player1.moves[key], weighting) # reward the player
            else: # the attemped play was invalid punish it
                for key in b.player1.moves:
                    aip.add(key, b.player1.moves[key], -weighting) # punish the player

            for key in b.player2.moves:
                aip.add(key, b.player2.moves[key], -weighting) # punish the losing player
